...
IL2RA_gene.fig <-
  ggplot(data = IL2RA) +
  geom_blank() +
  geom_segment(x=6052.652, xend=6104.288, y=1, yend=1, linewidth = 2) +
  annotate("text", label = "IL2RA",
           x= (6052.652 + 6104.288) / 2, y=1, hjust = 0.5, vjust = -1) +
  xlim(layer_scales(IL2RA_T1D.fig)$x$range$range) +
  ylim(0.75, 2) +
  ylab(NULL) + xlab(NULL) +
...
D1_bug <- list(
  ...
  MAF = IL2RA$maf.exposure,
  pos = IL2RA$pos.exposure,
  snp = IL2RA$pos.exposure, #results don't differ if SNP is used here
  sdY = 1)
...
check_dataset(D1_pass)
check_dataset(D2_pass)

coloc_IL2RA <- coloc.abf(D1_pass, D2_pass, p1 = 1e-4, p2 = 1e-4, p12 = 1e-5)
sensitivity(coloc_IL2RA, "H4 > 0.7")

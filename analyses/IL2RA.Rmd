...
plot_coloc(genechr = 10,
  ...
  ggsave(filename = "IL2RA.png", height = 8, width = 6)
```
...
```{r}
load("data/export/phenoscanner-functional-variants-missense.csv")
...
IL2RA_eqtl_rs72650666   <- IL2RA_eqtl %>% filter(SNP == "rs72650666")
MR_IL2RA_eqtl_rs72650666   <- TwoSampleMR::mr(IL2RA_eqtl_rs72650666)

MR_IL2RA_eqtl_rs72650666

exp(MR_IL2RA_eqtl_rs72650666$b)
exp(MR_IL2RA_eqtl_rs72650666$b - 1.96*MR_IL2RA_eqtl_rs72650666$se)
exp(MR_IL2RA_eqtl_rs72650666$b + 1.96*MR_IL2RA_eqtl_rs72650666$se)
```
...
## Coloc assuming 2 causal variants
```{r}

IL2RA_eqtl <- filter(IL2RA_eqtl, !is.na(eaf.outcome))


#get LD for the smaller data set
IL2RA_eqtl_ld  <- ld_matrix(
    variants = IL2RA_eqtl$SNP,
    pop = "EUR",
    plink_bin = genetics.binaRies::get_plink_binary(),
    bfile = "C:/Users/jajoko/Documents/plinkref/EUR",
    with_alleles = FALSE)
...
IL2RA_eqtl <- IL2RA_eqtl %>% 
filter(SNP %in% colnames(IL2RA_eqtl_ld))

#Set up data
D1 <- list(type = "quant", 
           pvalues = IL2RA_eqtl$pval.exposure, 
           beta = IL2RA_eqtl$beta.exposure, 
           varbeta = (IL2RA_eqtl$se.exposure) ^ 2,
           N = length(IL2RA_eqtl$samplesize.exposure), 
           MAF = ifelse(IL2RA_eqtl$eaf.outcome < 0.5, IL2RA_eqtl$eaf.outcome, 1- IL2RA_eqtl$eaf.outcome), 
           position = IL2RA_eqtl$pos.outcome,
           snp = IL2RA_eqtl$SNP,
           sdY = 1,
           LD = IL2RA_eqtl_ld)
           
D2 <- list(type = "cc", # case-control trait
           pvalues = IL2RA_eqtl$pval.outcome,
           beta = IL2RA_eqtl$beta.outcome, 
           varbeta = (IL2RA_eqtl$se.outcome) ^ 2,
           N = 18942+501638, # Case-control study (Chiou et al. 2021 Nature)
           s = 18942/(18942+501638), # N_case/(N_case+ N_ctrl)
           MAF = ifelse(IL2RA_eqtl$eaf.outcome < 0.5, IL2RA_eqtl$eaf.outcome, 1- IL2RA_eqtl$eaf.outcome),
           position = IL2RA_eqtl$pos.outcome,

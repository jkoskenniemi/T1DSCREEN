---
title: "TYK2"
author: "Tea Heikkilä, Emilia Kaiser & Jaakko Koskenniemi"
output: html_document
date: '2023-12-01'
editor_options:
  chunk_output_type: console
---

  Load packages and import data
```{r}
here::i_am("analyses/TYK2.Rmd")

source("code/01-Functions.R")

```


#eQTL

Load packages and import data
```{r}

TYK2_eqtl <- import_data("data/export_harmonization/TYK2_eqtl_TwoSampleMR.csv", 
                         "data/export_harmonization/TYK2_T1D_TwoSampleMR.csv", has_beta = TRUE)

```

## Initial Manhattan plots
```{r}
manhattan_plots(TYK2_eqtl, gene_start=10350533, gene_end=10380608, title = "TYK2")
TYK2_eqtl <- TYK2_eqtl %>% filter(pos.outcome > 10200000 & pos.outcome < 10700000) 
manhattan_plots(TYK2_eqtl, gene_start=10350533, gene_end=10380608, title = "TYK2")

#ggsave("figures/TYK2-eqtl-manhattan-fig.pdf",
#       height = 6, width = 6, units = "in")
```

## Coloc assuming 1 causal variant
```{r}
coloc_results_TYK2_eqtl <- analyze_coloc_eqtl(TYK2_eqtl)

```

## Regional manhattan plot

```{r}
load("data/export_LDmat/TYK2_eqtl_LDmat.rda")


plot_coloc(genechr = 19,
           fig_start = 10200000,
           fig_end = 10700000,
           coloc_results = coloc_results_TYK2_eqtl$coloc_D1D2,
           data = TYK2_eqtl,
           D1 = coloc_results_TYK2_eqtl$D1,
           D2 = coloc_results_TYK2_eqtl$D2,
           LDmat = TYK2)

ggsave(filename = "figures/TYK2.pdf", height = 8, width = 6)
```

## MR
```{r}

#Find top SNP
coloc_results_TYK2_eqtl$coloc_D1D2$results$snp[which.max(coloc_results_TYK2_eqtl$coloc_D1D2$results$SNP.PP.H4)] #rs34725611

#MR
TYK2_eqtl_rs34725611   <- TYK2_eqtl %>% filter(SNP == "rs34725611")
MR_TYK2_eqtl_rs34725611   <- TwoSampleMR::mr(TYK2_eqtl_rs34725611)

#F statistic
TYK2_eqtl_rs34725611 %>% 
  mutate(F = (beta.exposure/se.exposure)^2)

#MR results (unexponentiated)
MR_TYK2_eqtl_rs34725611

#Exponentiate betas and 95% CIs
exp(MR_TYK2_eqtl_rs34725611$b)
exp(MR_TYK2_eqtl_rs34725611$b - 1.96*MR_TYK2_eqtl_rs34725611$se)
exp(MR_TYK2_eqtl_rs34725611$b + 1.96*MR_TYK2_eqtl_rs34725611$se)


```


## Functional variants
```{r}

load("data/export_functional_variants/phenoscanner-entries.RDS")

#Find TYK2 missense variants
TYK2_missense <- functional_variants$TYK2$results %>% 
  filter(consequence == "missense") %>%  
  distinct(rsid) %>% pull()

TYK2_missense #rs55886939, rs34536443, rs35018800, rs55882956, rs12720356, 
              #rs139954108, rs2304256,rs188833543, rs55762744, rs144960992

#give betas and se for eQTL and T1D risk (note, this is not MR!)
TYK2_eqtl %>%
  filter(SNP %in% TYK2_missense) %>%
    mutate(Fstat = (beta.exposure/se.exposure)^2) %>% 
  select(SNP, beta.outcome, se.outcome, pval.outcome, beta.exposure, pval.exposure, Fstat) %>%
  mutate(across(!matches(c("SNP", "pval.outcome", "pval.exposure")), function(x) signif(x, digits = 2)))

#Select only rs12720356 and rs2304256 (as p for their exposure is < 10^-5) and run MR
TYK2_eqtl_funcvar_mr <- TYK2_eqtl %>%
  filter(SNP %in% c("rs12720356", "rs2304256")) %>%
  TwoSampleMR::mr_singlesnp() %>%
  mutate(exp_b = exp(b), 
         lo_95 = exp(b - (1.96 * se)),
         hi_95 = exp(b + (1.96 * se))) %>%
    mutate(across(starts_with(c("b", "se","p", "exp_b", "lo", "hi")), function(x) signif(x, digits = 2)))

  TYK2_eqtl_funcvar_mr %>% 
    select(-id.exposure, -id.outcome, -samplesize) %>% 
    knitr::kable()
```


```{r}
#LD matrix for interesting variants

load("data/export_LDmat/TYK2_tophits.rda")

knitr::kable(TYK2_SNPs_LD)

```


## pQTL

Per request of reviewers
```{r}
TYK2_pqtl <- import_data("data/export_harmonization/TYK2_prot_anno_TwoSampleMR.csv",
            "data/export_harmonization/TYK2_T1D_TwoSampleMR.csv", has_beta = TRUE)

TYK2_pqtl

coloc_results_TYK2_pqtl <- analyze_coloc_pqtl_nosensitivitity(TYK2_pqtl)
```



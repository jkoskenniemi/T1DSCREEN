...
TYK2_eqtl <- read_exposure_data("data/export/TYK2_eqtl_TwoSampleMR.csv", sep=",")
TYK2_T1D  <- read_outcome_data("data/export/TYK2_T1D_TwoSampleMR.csv", sep=",")

TYK2 <- harmonise_data(TYK2_eqtl, TYK2_T1D)
...
D2 <- list(
  ...
  snp = TYK2$pos.exposure) #to match the data, SNP works equally 
#well but doesn't print well

check_dataset(D1)
check_dataset(D2)

coloc_TYK2 <- coloc.abf(D1, D2, p1 = 1e-4, p2 = 1e-4, p12 = 1e-5)
sensitivity(coloc_TYK2, "H4 > 0.7")
```
...
TYK2_gene.fig <-
  ...
  geom_segment(x=37521.878, xend=37571.094, y=1, yend=1, linewidth = 2) +
  annotate("text", label = "TYK2",
           x= (10461.209 + 10491.352) / 2, y=1, hjust = 0.5, vjust = -1) +
  xlim(layer_scales(TYK2_T1D.fig)$x$range$range) +
  ylim(0.75, 2) +
  ylab(NULL) + xlab(NULL) +
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(),
        panel.grid.major.y = element_blank(),

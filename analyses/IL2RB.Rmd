...
IL2RB_prot <- read_exposure_data("data/export/IL2RB_prot_anno_TwoSampleMR.csv", sep=",")
IL2RB_T1D  <- read_outcome_data("data/export/IL2RB_T1D_TwoSampleMR.csv", sep=",")

IL2RB <- harmonise_data(IL2RB_prot, IL2RB_T1D)
...
fig_IL2RB_manhattan <- ggarrange(IL2RB_gene.fig, IL2RB_prot.fig, IL2RB_T1D.fig,
...
D1 <- list(
  ...
  Beta = IL2RB$beta.exposure,
  varBeta = IL2RB$se.exposure^2, 
...
D2 <- list(
  ...
  MAF = IL2RB$maf.outcome,  #prot used here in purpose
...
coloc_IL2RB <- coloc.abf(D1, D2, p1 = 1e-4, p2 = 1e-4, p12 = 1e-5)
...
# 
# save(ldmat_IL2RB, file = "data/IL2RB_LDmat-2023-04-25.RData")
...
Correct the orientation of effect alleles in IL2RB data frame 
```{r}
#remove SNPs from original (IL2RB) data that are not included in LD
#correlation matrix
IL2RB <- IL2RB %>% 
  filter(SNP %in% ldmat_IL2RB_snps$rsid)

#join SNP alignment data from the LD matrix to original (IL2RB) data 
...
IL2RB %>% filter(EA_ld != effect_allele.exposure & 
                   NEA_ld != other_allele.exposure) #no
IL2RB %>% filter(NEA_ld != other_allele.exposure & 
...
IL2RB %>% filter(harmonized == 1) #0

```

No variants were harmonized. Remove variants that are not found in IL2RB data frame
...
#Remove variants that are not found in IL2RB data frame
ldmat_IL2RB <- ldmat_IL2RB[rsid1 %in% IL2RB$SNP | 
                             rsid2 %in% IL2RB$SNP]
...
D3 <- list(
  ...
  beta = IL2RB$beta.exposure,
  varbeta = IL2RB$se.exposure^2, 
  pvalues = IL2RB$pval.exposure,
  n = IL2RB$samplesize.exposure,
  MAF = IL2RB$maf.exposure,
  position = IL2RB$pos.exposure,
  snp = IL2RB$SNP,
  LD = ldmat_IL2RB_corr,
  sdY = 1)
...
D4 <- list(
  type = "cc", # case-control trait
  beta = IL2RB$beta.outcome,
  varbeta = IL2RB$se.outcome^2,
  pvalues = IL2RB$pval.outcome,
  n = 18942+501638, # Case-control study (Chiou et al. 2021 Nature)
  s = 18942/(18942+501638), # N_case/(N_case+ N_ctrl)
  MAF = IL2RB$maf.outcome,
  LD = ldmat_IL2RB_corr,
  position = IL2RB$pos.outcome,
  snp = IL2RB$SNP)
...
### Run coloc.susie
```{r}
S3 <- runsusie(D3, n = IL2RB$samplesize.exposure[1])
...
IL2RB_clumped <- clump_data(IL2RB)
MR_IL2RB_res <- TwoSampleMR::mr(IL2RB_clumped)

MR_IL2RB_res_scatter <- mr_scatter_plot(MR_IL2RB_res, IL2RB_clumped)
MR_IL2RB_res_single <- mr_singlesnp(IL2RB_clumped)
MR_IL2RB_res_forest <- mr_forest_plot(MR_IL2RB_res_single)

exp(MR_IL2RB_res[3,]$b)
exp(MR_IL2RB_res[3,]$b)
exp(MR_IL2RB_res[3,]$b - 1.96*MR_IL2RB_res[3,]$se)
exp(MR_IL2RB_res[3,]$b + 1.96*MR_IL2RB_res[3,]$se)


exp(MR_IL2RB_res_single[2,]$b)
exp(MR_IL2RB_res_single[2,]$b - MR_IL2RB_res_single[2,]$se)
exp(MR_IL2RB_res_single[2,]$b + 1.96*MR_IL2RB_res_single[2,]$se)
